...
window.onload = function arrancarJuego() {
    ...
    $.ajax({
        ...
        success: function (jsn) {
            ...
            $.each(jsn, function (i) {
                var nomAux = this.nombre;
                var dificultadAux = this.dificultad;
                var naveAux = this.modeloNave;
                var lunaAux = this.modeloLuna;
                var obj = {nombreG: nomAux, dificultadG: dificultadAux, naveG: naveAux, lunaG: lunaAux}
                arrayConfig.push(obj);
                $("#select").append("<option>" + obj.nombreG + "</option>");
    ...
    $(function () { //onload...
        $("#guardar").click(function () { //onclick event
            ...
            var url = "servlet";
            var emess = "Error desconocido";
            var d = $("#dificultad").html(); //get dificultad from input
            var mL = $("#modeloLuna").html(); //get luna from input
            var mN = $("#modeloNave").html();
            var name = $("#nomConfig").val();

            $.ajax({
                method: "POST",
                url: url,
                data: {modeloNave: mN, modeloLuna: mL, nombre: name, dificultad: d},
                //data: {dificultad: d, nave: mN, lugar: mL},
                success: function (u) {
                    //var arrayConfig2 = [];
                    arrayConfig.push([name, d, mN, mL]);
    ...
    function cambiarDificultad(cambiarDificultad) {
        switch (cambiarDificultad) {
            case 1:
                gasolina = 50;
                gasolinaTotal = 50;
                document.getElementById("dificultad").innerHTML = "Media";
    ...
    document.getElementById("modeloLuna").onclick = function cambiarLuna() {
        switch (modeloLuna) {
            case 1:
    ...
    function cambiarModeloLuna(cambiarModeloLuna) {
        switch (cambiarModeloLuna) {
            case 1:
                document.getElementById("luna").src = "img/mod2luna.png";
                document.getElementById("modeloLuna").innerHTML = "Gris";
                modeloLuna = 2;
    ...
    function cambiarModeloNave(cambiarModeloNave) {
        switch (cambiarModeloNave) {
            case 1:
                document.getElementById("imgNave").src = "img/mod2nave.gif";
                document.getElementById("imgMotor").src = "img/mod2motor.gif";
                document.getElementById("modeloNave").innerHTML = "Modelo PodRacer";
    ...
    function cambio22() {
        var indexAux = $("#select option:selected").index();
        cambiarDificultad(indexAux);
        cambiarModeloLuna(indexAux);
        cambiarModeloNave(indexAux);
        
//        var indexAux = $("#select").val();
//        if (indexAux == "config1") {
//            cambiarDificultad(3);
//            cambiarModeloLuna(2);
//            cambiarModeloNave(2);
